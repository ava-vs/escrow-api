...
adminRoutes.put('/users/:id/balance', validateApiKey, async (c) => {
  try {
    ...
    // Set balance directly (not add to existing)
    await userService.setUserBalance(userId, amount);
...
// Create user (admin only) - alternative endpoint
adminRoutes.post('/users', validateApiKey, async (c) => {
  try {
    const { name, email, password, type = 'CUSTOMER', bio } = await c.req.json();
